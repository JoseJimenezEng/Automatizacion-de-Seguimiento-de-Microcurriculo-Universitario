    ...
    if (filteredData.length === 0) {
        ...
        filteredData.forEach((rowData) => {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                // Formatear fechas y horas según la columna
...
function displayWebhookData(data) {
    ...
    for (const groupId in data) {
        ...
        groupData.forEach((entry) => {
            ...
            // Botón de Coherente (antes era check)
            ...
            // Botón de Incoherente (antes era x)
...
function sendActionRequest(groupId, entry, color) {
    // Crear objeto con los datos a enviar, incluyendo el token de sesión
    const data = {
        ...entry,
        groupId: groupId,
        color: color,
    ...
    fetch(postUrl, {
        ...
        .then((result) => {
            showMessage(
                `<i class="fas fa-check-circle"></i> Acción ${color ? "incoherente" : "coherente"} enviada correctamente`,
                "success"
            );
        })
        .catch((error) => {
